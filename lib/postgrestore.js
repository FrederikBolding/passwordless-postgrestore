 ...
 * @constructor
 */
function PostgreStore(config, options) {
    if(!config){
    ...
    //config.max = this._options.pgstore.pgPoolSize || 5;

    this._client = new pg.Pool(config);
...
PostgreStore.prototype.storeOrUpdate = function(token, uid, msToLive, originUrl, callback) {
    ...
    bcrypt.hash(token, self._difficulty, function(err, hashedToken) {
        self._client.connect((err, client, done) => {
        if(err) {
            //done()
            return callback(err);
        }

        client.query('INSERT INTO ' + self._table + '(uid,token, origin, ttl) VALUES($1, $2, $3, $4)',[uid, hashedToken, originUrl, (Date.now() + msToLive)], function(err) {
            if(err){
                client.query('UPDATE ' + self._table + ' SET token=$1, origin=$2, ttl=$3 WHERE uid=$4',[hashedToken, originUrl, (Date.now() + msToLive), uid], function(err) {
                    done()
                    if(err){
                        callback(err);
                    }
...
PostgreStore.prototype.invalidateUser = function(uid, callback) {
    ...
    self._client.connect((err, client, done) => {
        client.query('DELETE FROM ' + this._table + ' WHERE uid=$1',[uid], function(err) {
            done()
            if(err){
                callback(err);
            }
...
PostgreStore.prototype.clear = function(callback) {
    ...
    var self = this;
    self._client.connect((err, client, done) => {
        client.query('DELETE FROM ' + this._table, function(err) {
            //this._client.release()
            done()
            if(err){
                callback(err);
...
PostgreStore.prototype.length = function(callback) {
    ...
    var self = this;
    self._client.connect((err, client, done) => {
        client.query('SELECT COUNT(uid) FROM ' + this._table, function(err, result) {
            done()
            if(err){
                callback(err);
            }
